...
- **Internet exposure.** The built-in login is a single shared credential. It returns `429` after five failed attempts in an hour from one address, counting an IPv6 /64 as one address. That address can try again fifteen minutes after its last failed attempt, and each further failure starts that wait again. Ten failures against the username from anywhere make every address that has itself failed wait a minute; the wait does not grow with further guesses, and an address with no failures is never refused. The current-password check for a password change in Settings counts against the same limit. The login page shows the remaining wait; a successful login clears the relevant counters, and restarting Qobuz Librarian clears the in-memory lockout. This is appropriate for a trusted home LAN. If you expose the UI to the internet, put it behind a reverse proxy, VPN, or Tailscale. A minimal Caddy example:
