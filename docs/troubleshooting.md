...
| A volume shows **!** with "not writable by the container user" on **Settings → Diagnostics** | `PUID`/`PGID` do not match the host owner. Run `chown -R $(id -u):$(id -g) ./music ./staging`, or set them in `.env`. |
