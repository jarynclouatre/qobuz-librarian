...
def test_executor_gap_fill_retires_the_moved_catalogue_row(monkeypatch, tmp_path):
    import os
    import sqlite3
    ...
    album_dir.mkdir(parents=True)
    old_track = album_dir / "01 - Old.flac"
    old_track.write_bytes(b"old")
    monkeypatch.setattr(executor.cfg, "MUSIC_ROOT", music)
    monkeypatch.setattr(executor.cfg, "UPGRADE_BACKUP_DIR", tmp_path / "backups")
    backup = bkmod.backup_gap_fill_files([old_track], album_dir)
    assert backup is not None and backup.complete and not old_track.exists()

    replacement = album_dir / "01 - New.flac"
    replacement.write_bytes(b"new")
    database = tmp_path / "beets.db"
    monkeypatch.setattr(executor.cfg, "BEETS_DB_PATH", database)
    with sqlite3.connect(database) as connection:
        ...
        connection.executemany(
            ...
            [
                (1, os.fsencode(old_track), 10),
                (2, os.fsencode(replacement), 20),
            ],
        )
    connection.close()

    monkeypatch.setattr(executor, "find_album_dir_filesystem", lambda _album: album_dir)
    monkeypatch.setattr(executor, "folder_holds_all_tracks", lambda *_a, **_k: True)
    item = {
        "album": {
            "id": "A",
            "artist": {"name": "Artist"},
...
def test_pre_import_hooks_keep_the_complete_downsample_outcome(
        monkeypatch, tmp_path):
    from qobuz_librarian import config as cfg
    from qobuz_librarian.completion import SourceTransitionKind
    from qobuz_librarian.queue import executor
    ...
    monkeypatch.setattr(executor, "HAVE_DOWNSAMPLE", True)
    monkeypatch.setattr(executor, "relocate_disc_album_artwork", lambda _d: None)
    monkeypatch.setattr(
        executor,
        "downsample_dir",
        lambda directory, **_kwargs: outcomes[directory],
    )
    monkeypatch.setattr(executor, "_run_lyric_hook", lambda _d: None)
    transitions = []

    prepared = executor._run_pre_import_hooks_for_dirs(
        [first, second],
        Namespace(no_downsample=False),
        on_sources_changed=transitions.append,
    )
    lyric_sigs, resampled = prepared

    assert lyric_sigs == []
    assert resampled == 3
    assert prepared.errors == 3
    assert prepared.saved_bytes == 150
    assert prepared.flush_warnings == 1
    assert prepared.cancelled is True
    assert transitions == [
        SourceTransitionKind.DOWNSAMPLE,
        SourceTransitionKind.LYRICS_TAG,
    ]


def test_durable_completion_line_matches_the_result(monkeypatch, tmp_path):
    from qobuz_librarian.queue import executor

    fetch_log = []
    monkeypatch.setattr(executor, "log_fetch", fetch_log.append)

    def completed(**outcome):
        item = {
            "album": {"id": "1", "title": "Album", "tracks": {"items": []}},
            "n_ok": 0,
            "n_fail": 0,
            "n_lossy": 0,
            "auto_upgrade": False,
        ...
        result = executor._durable_completed_result(item, tmp_path)
        return result, executor._durable_completion_line(result, 4)

    downloaded, clean_line = completed(n_ok=1)
    partial, partial_line = completed(n_ok=1, n_fail=1)
    failed, failed_line = completed(n_fail=1)
    caveat, caveat_line = completed(
        n_ok=1,
        downsample_errors=2,
        downsample_saved_bytes=512,
        downsample_flush_warnings=1,
        downsample_cancelled=True,
    )

    assert downloaded["result"] == "downloaded"
    assert clean_line[0] == executor.C.GREEN and "✓" in clean_line[1]
    assert partial["result"] == "partial"
    assert partial_line[0] == executor.C.YELLOW and "1 unresolved" in partial_line[1]
    assert failed["result"] == "failed"
    assert failed_line[0] == executor.C.RED and "✗" in failed_line[1]
    assert caveat["result"] == "partial"
    assert caveat_line[0] == executor.C.YELLOW
    assert "2 downsample failed" in caveat_line[1]
    assert "1 flush warning" in caveat_line[1]
    assert "downsample stopped" in caveat_line[1]
...
def test_queue_keeps_hook_caveats_and_runs_post_download_recheck(
        monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        ...
        lambda _dirs, _args: executor.PreImportHooksResult(
            [], 0, errors=1, saved_bytes=256
        ),
    )
    ...
    assert results and results[0]["result"] == "partial"
    assert results[0]["downsample_errors"] == 1
    assert results[0]["downsample_saved_bytes"] == 256
    assert fetch_log[-1]["downsample_saved_bytes"] == 256
