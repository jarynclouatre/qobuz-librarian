...
Every page says what the app recorded, a phone shows more of each screen,
and the web app is laid out in modules a contributor can find their way
around.

- Search keeps the Album tab to albums whose title matches, says how many results matched only the artist name and links the Artist tab for them, heads each result list with the query, and clears the list when the tab changes. Results, reviews and History are rows with one line between them, and on a phone the first result sits high enough for four to fit on the screen.
- A failed download names the cause the app recorded: a folder that changed, a backup or catalogue write that failed, a full disk, a track that arrived incomplete, or a rate limit. A rejected token, a missing user ID and an account restriction each get their own sentence. A review's picks and its name stay through Cancel and a restart, New releases and Restore included, a refresh that empties a review closes it, and a scan worker carries on when a review cannot be saved back.
- History records terminal downloads with the same status and attention as web downloads, titles an approved New releases run as a download, keeps a failed migration's recovery through Clear history, and marks a Repair scan that could not read every artist. Write controls grey out while writes are paused, a queued job's Remove becomes Cancel once it starts, the Library offers Resume scan after an interruption, and each message that names a page links to it.
- Settings keeps a timer set in .env through a save, gives each field one sentence of help, lays its sections out as one list with one gold button, and shows the Diagnostics marks legibly. The rarely changed fields sit in an Advanced drawer at the end of Behaviour, and Mode is the last section. The beets singleton path setting is removed, since the import never used it. The first Downsample asks before it deletes hi-res originals.
- The look: one gold action per screen with no gradients or glows, text that passes contrast in both themes, 44 px targets on a phone, one tab style, one count pill, one icon weight, review tools on two rows on a phone, and Discard red only where a review cannot be brought back.
- The terminal fits its prompts and help to the window, asks again when a yes or no answer is neither, cancels at an unanswered prompt, ends each mode with one closing line, treats Ctrl-C in Migrate and Artist as an interrupt, warns before Upgrade and Repair replace files, names the holder of the run lock, and runs the Upgrade walk without a web scan.
- The web routes live in modules by area, with the shared code in modules named for what they do; swallowed failures are logged, copied helpers share one module, and the job list API returns one row shape. The configuration, CLI and API pages match the app, and the compose and .env comments say what each switch does.
- Docker moves to uvicorn 0.54.0.
...
saved reviews, authentication, offline use, and library maintenance.

- Collection Backup writes a portable JSON inventory after library scans, keeps five dated copies, and protects the last good copy when a scan is unexpectedly empty or much smaller. Settings can write or download a backup on demand, and the bundled Compose setup gives backups their own host folder.
- Restore from a backup builds one review of albums missing from the current library. It prefers saved Qobuz links and track identifiers, requires confirmation before downloading, and rechecks the target library before files can change.
- Discover adds similar-artist suggestions, library genres, artist search, and saved Qobuz albums once a Last.fm API key is configured. Results resolve back to Qobuz and respect the app's release and ownership rules.
- The terminal gains a Settings screen, grouped command-line help, standard Ctrl-C exit status, and activity records shared with Queue and History. New Releases is now web-only, where its review and automatic refresh are available.
