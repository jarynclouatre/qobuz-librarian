...
def test_interrupted_consolidation_is_not_reported_as_zero_removed(
        monkeypatch, tmp_path, caplog):
    from qobuz_librarian import config as cfg
    from qobuz_librarian.modes import process as proc

    staging = tmp_path / "staging"
    staged_album = staging / "Artist" / "Album"
    staged_album.mkdir(parents=True)
    ...
    album = {
        ...
        "tracks": {"items": [{"id": 1, "title": "Track", "track_number": 1}]},
    }
    ...
    monkeypatch.setattr(proc, "compute_missing", lambda tracks, _existing: (tracks, []))
    monkeypatch.setattr(proc, "find_album_dir_filesystem", lambda _album: final_dir)
    monkeypatch.setattr(proc, "staging_preflight", lambda _args: None)
    monkeypatch.setattr(proc, "snapshot_staging", lambda: set())
    monkeypatch.setattr(proc, "validated_staged_album_dirs",
                        lambda _result: [staged_album])
    monkeypatch.setattr(proc, "is_cancel_requested", lambda: False)
    monkeypatch.setattr(proc, "_pre_import_staging_hooks",
                        lambda _args, _dirs=None: ([], 0))
    monkeypatch.setattr(proc, "beets_import_paths", lambda *_args, **_kwargs: True)
    ...
    fetch_log = []
    monkeypatch.setattr(proc, "log_fetch", fetch_log.append)
    monkeypatch.setattr(proc, "warn_if_download_truncated",
    ...
    monkeypatch.setattr(proc, "verify_and_recover",
                        lambda *_args, **_kwargs: {
                            ...
                            "recovered": False,
                            "retried": False,
                            "staged_dirs": [staged_album],
                        })
    monkeypatch.setattr(proc, "track_signatures_for_album_dirs",
                        lambda _dirs: ["signature"])
    monkeypatch.setattr(proc, "find_album_dir_by_track_signatures",
                        lambda _signatures: final_dir)
    ...
    result = proc.process_album(
        album,
        _args(consolidate=True),
        token="tok",
    )

    assert result["result"] == "partial"
    assert result["consolidation_interrupted"] is True
    assert "consolidated: stopped early" in caplog.text
    assert "consolidated:   0 sibling tracks removed" not in caplog.text
    assert fetch_log[-1]["consolidated"] is False
    assert fetch_log[-1]["consolidation_interrupted"] is True
