...
- A Discover tab appears once a Last.fm API key is saved in Settings. It suggests artists like the ones you own, naming the artists in your library that vouch for each; browses the genres of your library; searches for artists similar to a name you type; and lists the albums you saved on Qobuz that are not on disk yet. Every card is an album Qobuz can serve, and short single-track releases are left out.
...
- A stranger's failed sign-ins can no longer lock you out. Behind a reverse proxy every visitor can arrive as the same address, so someone else's guesses counted against you. The app now says so at startup and names the setting that fixes it, and a browser that is already signed in is never counted at all.
...
- The terminal has a Settings mode, says when artist mode is listing albums you dismissed, and refuses to sit waiting at a prompt when nothing is there to answer it.
