...
Repair, after the first real scan of a whole library, plus a search row that named the wrong record. A wrong ISRC tag was enough to make Repair offer to replace a healthy file, and an approved repair deletes your original with no grace period, so take this one.

- Search stops putting one pressing's name over another pressing's numbers. When a record has several releases they collapse into one row with the rest tucked underneath, and those tucked rows were drawn with the top row's title and an invented "1968 edition" label, so The Doors' plain 11-track Waiting for the Sun was offered as the 25-track 50th Anniversary Deluxe Edition it had been grouped under, down to the download confirmation naming the deluxe. The "N of M" badge counted a different release again: a row could read "25 tracks" and "3 of 11" together, promising eight tracks to go while its Download button would have pulled twenty-two into your album folder. Every release now carries its own title, and the row counts the one it would actually fetch.
- "Select all" on the search page picks albums, not every pressing of them. With a versions fold open it swept the alternate pressings in as well, so one record could go to the queue three times over and land in one folder; the duplicate check only refuses the same album id, so it refused none of them. Turning the filter on also counted those pressings as albums, which made "Hide owned" raise the total instead of lowering it.
- Repair no longer calls a file damaged because a different song is longer. Every file is matched to a Qobuz recording by its ISRC tag and a big shortfall was read as truncation, but a wrong tag resolves to another track entirely: a healthy 128 second song was measured against a 407 second recording it has nothing to do with, flagged as truncated, and offered for replacement. Approving that would have put the other recording in the album wearing the original's title and tags. A match whose title does not read like the file's own is no longer treated as evidence of anything, and a file that will not decode is still caught whatever its tag says.
- The repair sweep lists what it flags. It opened by promising that problems appear in the log below and then logged only the albums it could not scan at all, so an hour-long scan looked like it had found nothing and then produced a review out of nowhere. Flagged albums are now named as they are found, review rows are labelled with the file on your disk instead of the recording it matched, and the card reports what was measured ("1 track 392s shorter than the catalogue version") rather than asserting damage it has not proved.
- A damaged file whose ISRC names a different song is no longer offered a one-track replacement. Damage is found without trusting the tag, which is right, but the replacement it lined up was still the song the wrong tag pointed at: approving it downloaded that other recording, put your file's title on it, checked it against its own tag, and deleted your original, because the substitute genuinely is that other song and agrees with itself. Those files are now offered a whole-album re-download instead, the same way as a damaged file with no usable tag at all, and you still confirm it.
- A repair no longer fails because another album contains a track of the same name. The download, the import and the placement were all correct on disk and the run still reported that it could not prove where the file had ended up, kept your original in a backup, and counted the album as unrepaired.
- A kept backup says what actually happened to your files. Every one of them read "1 of 1 original file kept while Repair stopped", even where the replacement was already sitting in the album, so the page said nothing had changed while Settings said the opposite about the same backup. It now gives the real reason and points at Restore.
- Albums flagged for a whole-album re-download appear in the repair log. The sweep promises that every problem is listed and these were listed nowhere, so six damaged files across three albums scrolled past as silence and then turned up in the review.
- The Repair page stops saying no scan has finished while showing you one that has. A scan that failed or was cancelled stays on the page on purpose, with the launcher underneath, and that launcher was reporting a first visit: no date for the last scan, and no offer to resume a sweep that had been interrupted.
- The documentation says what the app actually does. Repair's ISRC handling had moved on without it, and `AUTO_UPGRADE_ENABLED` was written up as a CLI-only compatibility option when it also reaches the web app, where an ordinary download can back up and replace a whole album folder with nothing asked.
