...
- Settings keeps a timer set in .env through a save, gives each field one sentence of help, lays its sections out as one list with one gold button, and shows the Diagnostics marks legibly. The beets singleton path setting is removed, since the import never used it. The first Downsample asks before it deletes hi-res originals.
...
- The web routes live in modules by area with shared state in one place, swallowed failures are logged, and copied helpers share one module. The configuration, CLI and API pages match the app, and the compose and .env comments say what each switch does.
